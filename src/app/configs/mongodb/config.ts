...
export const modConnection: Connection = mongoose.createConnection(modUrl);
...
const connectDB = async () => {
  try {
    ...
    const conn = await mongoose.connect(sysUrl);
    console.log(
      `MongoDB connected: ${conn.connection.host} (db: ${MONGO_DBNAME})`
    );

    await modConnection.asPromise();
    console.log(
      `MongoDB connected: ${modConnection.host} (db: ${MONGO_DBNAME_MOD})`
    );
  } catch (error) {
