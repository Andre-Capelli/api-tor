...
let PlanController = class PlanController extends tsoa_1.Controller {
    async getAll() {
        try {
            this.setStatus(200);
            return await new PlanService_1.PlanService().getAll();
        }
        catch (error) {
            this.setStatus(500);
            throw error;
    ...
    async getById(id) {
        try {
            const plan = await new PlanService_1.PlanService().getById(id);
            if (!plan) {
                this.setStatus(404);
    ...
    async create(body) {
        try {
            this.setStatus(201);
            const created = await new PlanService_1.PlanService().create(body);
    ...
    async update(id, body) {
        try {
            this.setStatus(200);
            await new PlanService_1.PlanService().update(id, body);
            return;
        }
        catch (error) {
            this.setStatus(500);
    ...
    async delete(id) {
        try {
            this.setStatus(200);
            await new PlanService_1.PlanService().delete(id);
            return;
        }
        catch (error) {
            this.setStatus(500);
...
__decorate([
    (0, tsoa_1.SuccessResponse)("200", "Plan"),
